...
RUN adduser --no-create-home -u edumqes -D $APPLICATION_USER
# Configure working directory
RUN mkdir /app && \
    chown -R $APPLICATION_USER /app
USER edumqes
COPY --chown=edumqes:edumqes --from=builder /opt/app/AvaliacaoCloudMinsait/target/minsait-0.0.1-SNAPSHOT.jar  /app/app.jar
COPY --chown=edumqes:edumqes --from=builder /opt/app/*.jar /app/.
WORKDIR /app
EXPOSE 8080
